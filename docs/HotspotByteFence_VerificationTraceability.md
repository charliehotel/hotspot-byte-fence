...
|---|---|---|
| Domain tests | `Tests/HotspotByteFenceCoreTests/` and `swift test` | Implemented core cases, including initial cross-file persistence, configuration archive, and preference transaction cases, pass with deterministic inputs and no system side effect; full `D-*` coverage remains pending |
...
| Fixture assets | `Tests/HotspotByteFenceCoreTests/Fixtures/HotspotByteFence/` | The initial counter fixture, typed persistence cases, configuration archive cases, and preference transaction cases validate their local oracles; the remaining fixture families and negative corpus are pending |
